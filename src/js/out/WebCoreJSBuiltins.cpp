...
const int s_transformStreamInternalsTransformStreamDefaultControllerPerformTransformCodeLength = 374;
...
const int s_transformStreamInternalsTransformStreamDefaultSinkWriteAlgorithmCodeLength = 817;
...
const int s_transformStreamInternalsTransformStreamDefaultSinkCloseAlgorithmCodeLength = 1017;
...
const int s_processObjectInternalsBindingCodeLength = 521;
static const JSC::Intrinsic s_processObjectInternalsBindingCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_processObjectInternalsBindingCode = "(function (bindingName){\"use strict\";if(bindingName!==\"constants\")@throwTypeError(\"process.binding() is not supported in Bun. If that breaks something, please file an issue and include a reproducible code sample.\");var cache=globalThis.Symbol.for(\"process.bindings.constants\"),constants=globalThis[cache];if(!constants){const{constants:fs}=@getInternalField(@internalModuleRegistry,15)||@createInternalModuleById(15);constants={fs,zlib:{},crypto:{},os:@Bun._Os().constants},globalThis[cache]=constants}return constants})\n";
...
const int s_moduleRequireCodeLength = 1078;
static const JSC::Intrinsic s_moduleRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_moduleRequireCode = "(function (id){\"use strict\";const existing=@requireMap.@get(id)||@requireMap.@get(id=@resolveSync(id,this.path,!1));if(existing)return @evaluateCommonJSModule(existing),existing.exports;if(id.endsWith(\".json\")||id.endsWith(\".toml\")||id.endsWith(\".node\"))return @internalRequire(id);let esm=@Loader.registry.@get(id);if(esm\?.evaluated&&(esm.state\?\?0)>=@ModuleReady){const mod2=esm.module,namespace=@Loader.getModuleNamespaceObject(mod2),exports=namespace.__esModule\?namespace:Object.create(namespace,{__esModule:{value:!0}});return @requireMap.@set(id,@createCommonJSModule(id,exports,!0)),exports}const mod=@createCommonJSModule(id,{},!1);@requireMap.@set(id,mod);var out=this.@require(id,mod);if(out===-1){try{out=@requireESM(id)}catch(exception){throw @requireMap.@delete(id),exception}if(esm=@Loader.registry.@get(id),esm\?.evaluated&&(esm.state\?\?0)>=@ModuleReady){const namespace=@Loader.getModuleNamespaceObject(esm.module);return mod.exports=namespace.__esModule\?namespace:Object.create(namespace,{__esModule:{value:!0}})}}return @evaluateCommonJSModule(mod),mod.exports})\n";
